...
N_SENS_RANGE = np.arange(135, 241, 5)
...
for i in range(len(N_SENS_RANGE)):
    for j in range(len(R_RANGE)):
        sens_array = BarbuteArraySL(N_SENS_RANGE[i], PARAMS['l_int'], PARAMS['l_ext'], R_inner=R_RANGE[j], R_outer=PARAMS['R_outer'], n_samp_layers=PARAMS['n_samp_layers'], n_samp_per_layer=PARAMS['n_samp_per_layer'], debug_fldr='/tmp/', **PARAMS['kwargs'])
